...
3. **Chunked Iterative Evidence Gathering (Map-Reduce):** 
   ...
   - For development evidence classification, enrichment is mandatory per UI story: inspect `description`, `comment`, `issuelinks`, and Jira remote links for PR/commit URLs even if AC mapping is already clear.
   - Attempt GitHub MCP lookup per UI story key; if lookup fails or returns no hits, continue and record evidence status using the existing `Evidence Unavailable` style.
   - Fetch GitHub PR detail only when needed for ambiguity resolution, but do not mark evidence unavailable until Jira-linked URL checks are completed.
